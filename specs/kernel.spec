...
# -- ro-ASD -- Kernel S --
#
# TR: BORE (Burst-Oriented Response Enhancer) Scheduler Yaması
# Bu yama, işlemci zamanlayıcısını (CPU Scheduler) optimize ederek sistemin
# tepki süresini (responsiveness) ve genel performansını artırır.
# Özellikle masaüstü kullanımı ve oyun performansında iyileştirme sağlar.
#
# EN: BORE (Burst-Oriented Response Enhancer) Scheduler Patch
# This patch optimizes the CPU Scheduler to improve system responsiveness
# and overall performance. It specifically targets improvements in desktop
# usage and gaming performance.
# -----------------------------------------------------------------------------
...
Patch410: 0001-bore-cachy.patch

# CachyOS Patches
Patch4: 0003-cachyos-base.patch
# Patch5: 0004-bbr3.patch
...
# 1. Fedora'nın Orijinal Yamaları
%if !%{nopatches}
...
# -- ro-ASD -- Kernel S --
#
# TR: BORE Scheduler Yamasının Uygulanması
# BORE Scheduler yaması %{_sourcedir} dizininden alınarak uygulanır.
# 'fuzz=3' ve 'ignore-whitespace' parametreleri ile küçük uyumsuzluklar
# giderilmeye çalışılır. '|| true' komutu yama uygulanamasa bile işlemin
# devam etmesini sağlar (dikkatli kullanılmalıdır).
# Ardından varsa düzeltme (fix) yaması uygulanır.
#
# EN: Applying BORE Scheduler Patch
# The BORE Scheduler patch is applied from the %{_sourcedir} directory.
# Parameters like 'fuzz=3' and 'ignore-whitespace' are used to handle
# minor incompatibilities. The '|| true' command allows the process to
# continue even if the patch fails (use with caution).
# Subsequently, a fix patch is applied if available.
# -----------------------------------------------------------------------------
# 2. Ana BORE Yaması (Hataları yoksayarak uygula, || true ile devam et)
# 2. Ana BORE Yaması
%patch -P 410 -p1

# 3. Fix Yaması (Hata veren yerleri düzelten yama)
# patch -p0 < %{_sourcedir}/0002-bore-fix.patch

# 4. CachyOS Base Patch (Clear Linux optimizations + more)
%patch -P 4 -p1

# 5. BBRv3 Patch
# patch -p1 < %{_sourcedir}/0004-bbr3.patch

# -----------------------------------------------------------------------------
# -- ro-ASD -- Kernel S --
#
# TR: Kernel Test Yaması
# Bu yama, kernel üzerinde geliştirme yapılırken veya yeni özellikler
# test edilirken kullanılır. Sadece test amaçlıdır, prodüksiyon ortamında
# kullanılmamalıdır.
#
# EN: Kernel Test Patch
# This patch is used for testing new features or during kernel development.
# It is intended for testing purposes only and should not be used in a
...
%endif
    ...
    KCFLAGS="%{?kcflags} -O3 -march=x86-64-v3"
}
